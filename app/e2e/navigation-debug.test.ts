...
test.describe('🔍 Navigation Debug', () => {
  ...
  test('Debug shifts route navigation issue', async ({ page }) => {
    ...
    for (const route of routes) {
      ...
      if (route.path === '/shifts') {
        ...
        const hasSignInMessage = await page.getByText('Please sign in to view available shifts').isVisible();
  ...
  test('Alternative body visibility check', async ({ page }) => {
    ...
    const checks = {
      ...
      specificContent: await page.getByText('Available Shifts').isVisible().catch(() => false),
      signInMessage: await page.getByText('Please sign in').isVisible().catch(() => false)
    };
  ...
  test('Debug shifts route with console monitoring', async ({ page }) => {
    ...
    const cardContent = await page.locator('[data-testid], .text-center, text="Please sign in"').count();
