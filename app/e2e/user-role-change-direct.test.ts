...
test.describe('User Role Change - Direct API Test', () => {
	...
	test('should successfully change user role via direct API call', async ({ request }) => {
		...
		try {
			...
			execSync(`sqlite3 "../night-owls.test.db" "UPDATE users SET role = 'admin' WHERE phone = '+27123456789';"`);
		} catch (error) {
		...
		console.log('Role change response headers:', await roleChangeResponse.allHeaders());
	...
	test('should log detailed backend response for role change failure', async ({ request }) => {
		...
		const unauthResponse = await request.put(`http://localhost:8080/api/admin/users/${existingUserId}`, {
			data: {
				phone: existingUserPhone,
				name: existingUserName,
				role: 'admin'
		...
		const invalidTokenResponse = await request.put(`http://localhost:8080/api/admin/users/${existingUserId}`, {
			headers: {
