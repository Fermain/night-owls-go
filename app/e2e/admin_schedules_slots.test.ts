...
test.describe('Admin Schedules Slots Page', () => {
	test('should allow selecting and re-selecting a date range in DateRangePicker', async ({ page }) => {
		await page.goto('/admin/schedules/slots');

		// The DateRangePicker trigger button
		const datePickerButton = page.locator('button[data-popover-trigger][aria-haspopup="dialog"]:has(svg.lucide-calendar)');
		...
		const firstDayOfMonth = page.locator('div[data-range-calendar-day]:not([data-outside-month])').filter({ hasText: '1' }).first();
		const tenthDayOfMonth = page.locator('div[data-range-calendar-day]:not([data-outside-month])').filter({ hasText: '10' }).first();
		...
		// Pick 15th and 20th
		const fifteenthDayOfMonth = page.locator('div[data-range-calendar-day]:not([data-outside-month])').filter({ hasText: '15' }).first();
		const twentiethDayOfMonth = page.locator('div[data-range-calendar-day]:not([data-outside-month])').filter({ hasText: '20' }).first();
