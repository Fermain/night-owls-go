...
test.describe('Authentication Flow', () => {
	...
	test('should complete full registration and login flow', async ({ page }) => {
		...
		await expect(page).toHaveURL(`/login?phone=${encodeURIComponent(testPhone)}&name=${encodeURIComponent(testName)}`);
		...
		await expect(page.getByText('Admin Dashboard').or(page.getByText('Dashboard'))).toBeVisible({ timeout: 10000 });
