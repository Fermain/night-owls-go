...
import { DatabaseHelper, AuthTestHelper } from './test-utils';
...
test.describe('User Role Change', () => {
	...
	test.beforeEach(async ({ page }) => {
		...
		dbHelper.updateUserRole(existingUserId, 'guest');
		...
		dbHelper.createUser(tempAdminPhone, tempAdminName, 'admin');
	...
	test('should log detailed debugging information', async ({ page }) => {
		...
		const formData = await page.evaluate(() => {
			...
			return {
				...
				userObject: (window as typeof window & { selectedUser?: unknown }).selectedUser
			};
		});
