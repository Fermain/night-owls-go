...
<DropdownMenu.Root>
	...
	<DropdownMenu.Content class="w-80" align="end">
		<div class="flex items-center justify-between px-2 py-1.5">
			...
			{#if $badgeCount > 0}
				<Button variant="ghost" size="sm" onclick={handleMarkAllRead} class="h-6 px-2 text-xs">
		...
		<div class="max-h-80 overflow-y-auto">
			...
			{:else}
				{#each $notificationState.notifications as notification (notification.id)}
					...
					<DropdownMenu.Item
						class="flex items-start space-x-3 p-3 cursor-pointer hover:bg-muted/50 {!notification.read
							? 'bg-muted/30'
							: ''}"
					...
					>
						<div class="flex-shrink-0 mt-0.5">
							<IconComponent
								class="h-4 w-4 {!notification.read ? 'text-primary' : 'text-muted-foreground'}"
							/>
						</div>
						...
						<div class="flex-1 min-w-0">
							<div class="flex items-start justify-between">
								<p
									class="text-sm font-medium leading-none {!notification.read
										? 'text-foreground'
		...
		{#if $notificationState.notifications.length > 0}
			...
			<DropdownMenu.Item class="justify-center text-center py-2">
				<a href="/broadcasts" class="w-full">
					<Button variant="ghost" size="sm" class="text-xs w-full">View all messages</Button>
				</a>
			</DropdownMenu.Item>
		{/if}
