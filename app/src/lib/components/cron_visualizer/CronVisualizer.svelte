            ...
            try {
                ...
                const getValuesFromField = (field: CronField, fieldName: string): number[] => {
                    if (field && typeof (field as any).getValues === 'function') {
                        ...
                        console.warn(`Could not directly access ${fieldName} values via .getValues() or .values, attempting iteration as fallback.`);
                ...
                if (parts[4] === '*') {
                    ...
                    activeDayOfWeekIndices = dayOfWeekValues.map((d: number) => {
    ...
    {:else if cronExpr && cronExpr.trim() !== ""}
        <div class="flex flex-wrap gap-1 items-center">
            ...
            {#if displayedMonths.length === 0 && displayedDays.length === 0 && displayedHoursAMPM.length === 0 && !error}
                <p class="italic">Expression implies all instances or is too broad for this condensed view.</p>
            {/if}
        </div>
    {:else if !cronExpr || cronExpr.trim() === ""}
