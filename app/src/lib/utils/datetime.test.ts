...
describe('getTimeUntil', () => {
	...
	it('should return "in Xh" for times over 60 minutes away', () => {
		const now = new Date('2024-01-01T10:00:00Z');
		vi.setSystemTime(now);

		const futureTime = '2024-01-01T12:00:00Z';
		expect(getTimeUntil(futureTime)).toBe('in 2h');
	});
	...
	it('should return "Now" for current time', () => {
		...
		expect(getTimeUntil(now.toISOString())).toBe('Now');
	});
	...
	it('should handle edge cases near time boundaries', () => {
		...
		// Exactly 1 hour
		expect(getTimeUntil('2024-01-01T11:00:00Z')).toBe('in 1h');

		// Just over 1 hour
		expect(getTimeUntil('2024-01-01T11:01:00Z')).toBe('in 1h');
	});
});
