...
func (h *AdminDashboardHandler) GetDashboardHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.ErrorContext(r.Context(), "Failed to get dashboard metrics", "error", err)
		RespondWithError(w, http.StatusInternalServerError, "Failed to get dashboard metrics", h.logger, "error", err.Error())
		return
	}
